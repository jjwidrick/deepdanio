#' Plot tail angular velocity vs movement time.
#'
#' Create a tail angular velocity plot using
#'     the results df.
#'
...
plot_tail_ang_vel <- function(df, settings, ER_plot_theme) {
  ...
  # make plot
  plot <- ggplot(plot.df, aes(x=move.ms, y=tail.vel.deg.ms)) +
               ...
               ggtitle("tail curvature angular velocity")
